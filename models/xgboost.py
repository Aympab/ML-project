...
def main(njobs):
    X_data = pd.read_csv("data/starting_kit/data/protein_train.data", sep=" ", header=None)
    y_data = pd.read_csv("data/starting_kit/data/protein_train.solution", sep=" ", header=None)

    X_test_data = pd.read_csv("data/starting_kit/data/protein_test.data", sep=" ", header=None)
    X_valid_data = pd.read_csv("data/starting_kit/data/protein_valid.data", sep=" ", header=None)
    output = open('GridSerachResultsEduardo.txt', 'w')
    ...
    clas = GridSearchCV(xgb.XGBClassifier(), tuned_parameters, cv=4, n_jobs=-njobs)
    ...
    clas_rf = GridSearchCV(RandomForestClassifier(), tuned_parameters, cv=4, n_jobs=njobs)
    ...
    svm_clas = GridSearchCV(SVC(), tuned_parameters, cv=4, n_jobs=njobs)
    ...
    clas_dt = GridSearchCV(DecisionTreeClassifier(), tuned_parameters, cv=4, n_jobs=njobs)
    ...
    gpclas = GridSearchCV(GaussianProcessClassifier(), tuned_parameters, cv=4, n_jobs=njobs)
