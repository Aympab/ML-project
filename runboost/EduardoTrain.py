...
X_data2 = X_data.iloc[:10].copy()
y_data2 = y_data.iloc[:10].copy()
...
clas = GridSearchCV(xgb.XGBClassifier(), tuned_parameters, cv=6, n_jobs=-1)
...
clas_rf = GridSearchCV(RandomForestClassifier(), tuned_parameters, cv=6, n_jobs=-1)
...
model = GridSearchCV(KNeighborsClassifier(), tuned_parameters, cv=6, n_jobs=-1)
...
svm_clas = GridSearchCV(SVC(), tuned_parameters, cv=6, n_jobs=-1)
...
clas_dt = GridSearchCV(DecisionTreeClassifier(), tuned_parameters, cv=6, n_jobs=-1)
...
gpclas = GridSearchCV(GaussianProcessClassifier(), tuned_parameters, cv=6, n_jobs=-1)
